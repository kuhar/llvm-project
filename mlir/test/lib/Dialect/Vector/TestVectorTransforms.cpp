...
struct TestVectorGatherEmulation
    : public PassWrapper<TestVectorGatherEmulation,
                         OperationPass<func::FuncOp>> {
  MLIR_DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(TestVectorGatherEmulation)

  StringRef getArgument() const final { return "test-vector-gather-emulation"; }
  StringRef getDescription() const final {
    return "Test emulator patterns that lower the gather op in the vector "
           "conditional loads";
  }
  void getDependentDialects(DialectRegistry &registry) const override {
  ...
  void runOnOperation() override {
    RewritePatternSet patterns(&getContext());
    populateVectorGatherEmulationPatterns(patterns);
    (void)applyPatternsAndFoldGreedily(getOperation(), std::move(patterns));
  }
};
...
void registerTestVectorLowerings() {
  ...
  PassRegistration<TestVectorGatherEmulation>();
}
} // namespace test
} // namespace mlir
