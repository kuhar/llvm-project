...
struct ConvertAddI final : OpConversionPattern<arith::AddIOp> {
  ...
  LogicalResult
  matchAndRewrite(arith::AddIOp op, arith::AddIOpAdaptor adaptor,
...
struct ConvertConstant final : OpConversionPattern<arith::ConstantOp> {
  ...
  LogicalResult
  matchAndRewrite(arith::ConstantOp op, arith::ConstantOpAdaptor,
                  ConversionPatternRewriter &rewriter) const override {
...
struct ConvertExtSI final : OpConversionPattern<arith::ExtSIOp> {
  ...
  LogicalResult
  matchAndRewrite(arith::ExtSIOp op, arith::ExtSIOpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    auto &typeConverter = *getTypeConverter<WideIntEmulationConverter>();
    ...
    Value extended = rewriter.createOrFold<arith::ExtSIOp>(loc, newOperandTy,
                                                           adaptor.getIn());
    ...
    Value operandZeroCst =
        rewriter.create<arith::ConstantOp>(loc, operandZeroAttr);
    Value signBit = rewriter.create<arith::CmpIOp>(
        loc, arith::CmpIPredicate::slt, extended, operandZeroCst);
    Value signValue =
        rewriter.create<arith::ExtSIOp>(loc, newOperandTy, signBit);

    Value vecZeroCst = rewriter.create<arith::ConstantOp>(
...
struct ConvertExtUI final : OpConversionPattern<arith::ExtUIOp> {
  ...
  LogicalResult
  matchAndRewrite(arith::ExtUIOp op, arith::ExtUIOpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    auto &typeConverter = *getTypeConverter<WideIntEmulationConverter>();
    if (!typeConverter.isLegal(op.getIn().getType()))
      return failure();
    ...
    Value extended = rewriter.createOrFold<arith::ExtUIOp>(
        op->getLoc(), newOperandTy, adaptor.getIn());
    Attribute zeroAttr =
        DenseElementsAttr::get(newTy, APInt::getZero(newBitWidth));
    Value zeroCst = rewriter.create<arith::ConstantOp>(op->getLoc(), zeroAttr);
    rewriter.replaceOpWithNewOp<vector::InsertOp>(
...
struct ConvertMulI final : OpConversionPattern<arith::MulIOp> {
  ...
  LogicalResult
  matchAndRewrite(arith::MulIOp op, arith::MulIOpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    Location loc = op->getLoc();
    auto &typeConverter = *getTypeConverter<WideIntEmulationConverter>();
    ...
    Value lowMask = rewriter.create<arith::ConstantOp>(loc, lowMaskAttr);
    ...
    for (unsigned i = 0, e = lhsDigits.size(); i != e; ++i) {
      for (unsigned j = 0; i + j != e; ++j) {
        ...
        if (i + j + 1 != e) {
          Value overflow = rewriter.createOrFold<arith::AddIOp>(
    ...
    Value vecZeroCst = rewriter.create<arith::ConstantOp>(
        loc, getScalarOrSplatIntegerAttr(newTy, 0));
    Value ins0 =
...
struct ConvertShRUI final : OpConversionPattern<arith::ShRUIOp> {
  ...
  matchAndRewrite(arith::ShRUIOp op, OpAdaptor adaptor,
    ...
    Value elemBitWidth = rewriter.create<arith::ConstantOp>(
...
struct ConvertTruncI final : OpConversionPattern<arith::TruncIOp> {
  ...
  LogicalResult
  matchAndRewrite(arith::TruncIOp op, arith::TruncIOpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    auto &typeConverter = *getTypeConverter<WideIntEmulationConverter>();
    ...
    Value truncated = rewriter.createOrFold<arith::TruncIOp>(
        op->getLoc(), op.getType(), extracted);
