...
bool ElfView::updateKernelMetadataVgprCounts(
  ...
  if (MetadataCacheState == KernelMetadataCacheState::Metadata) {
    for (const StringMapEntry<unsigned> &Required : RequiredVgprs) {
      StringMap<std::optional<unsigned>>::iterator Cached =
          KernelVgprCountCache.find(Required.first());
      if (Cached == KernelVgprCountCache.end())
        KernelVgprCountCache.try_emplace(Required.first(), Required.second);
      else if (!Cached->second || Required.second > *Cached->second)
        Cached->second = Required.second;
    }
  }
...
ElfView::getKernelMaxFlatWorkgroupSize(StringRef KernelName) const {
  ...
  StringMap<std::optional<unsigned>>::const_iterator Cached =
      KernelMaxFlatWorkgroupSizeCache.find(KernelName);
  return Cached == KernelMaxFlatWorkgroupSizeCache.end() ? std::nullopt
                                                         : Cached->second;
}
...
ElfView::getKernelMetadataVgprCount(StringRef KernelName) const {
  ...
  if (MetadataCacheState != KernelMetadataCacheState::Metadata)
    return std::nullopt;
  StringMap<std::optional<unsigned>>::const_iterator Cached =
      KernelVgprCountCache.find(KernelName);
  return Cached == KernelVgprCountCache.end() ? std::nullopt : Cached->second;
}
...
ElfView::getKernelWavefrontSize(StringRef KernelName) const {
  ...
  if (MetadataCacheState != KernelMetadataCacheState::Metadata)
    return std::nullopt;
  StringMap<std::optional<unsigned>>::const_iterator Cached =
      KernelWavefrontSizeCache.find(KernelName);
  return Cached == KernelWavefrontSizeCache.end() ? std::nullopt
                                                  : Cached->second;
}
    ...
    for (const ELFT::Phdr &Phdr : *PhdrsOrErr) {
      ...
      for (ELFT::Note Note : File.notes(Phdr, Err)) {
        ...
        for (msgpack::DocNode &KNode : KernelArray) {
          ...
          KernelVgprCountCache.try_emplace(
              Name,
              readOptionalUnsignedMetadataField(KMap, Name, ".vgpr_count"));
          KernelMaxFlatWorkgroupSizeCache.try_emplace(
              Name, readOptionalUnsignedMetadataField(
                        KMap, Name, ".max_flat_workgroup_size"));
          KernelWavefrontSizeCache.try_emplace(
              Name,
              readOptionalUnsignedMetadataField(KMap, Name, ".wavefront_size"));
          KernelClusterDimsCache.try_emplace(
              Name, readOptionalClusterDimsMetadataField(KMap, Name));
        }
      }
      if (Err) {
...
ElfView::getKernelClusterDims(StringRef KernelName) const {
  ...
  if (MetadataCacheState != KernelMetadataCacheState::Metadata)
    return std::nullopt;
  StringMap<std::optional<KernelClusterDims>>::const_iterator Cached =
      KernelClusterDimsCache.find(KernelName);
  return Cached == KernelClusterDimsCache.end() ? std::nullopt : Cached->second;
}
